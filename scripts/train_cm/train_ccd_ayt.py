...
def main(cfg: DictConfig):
    ...
    if rank == 0:
        logger.info('Loading classifier checkpoint from [{}]'.format(cfg['classifier']['ckpt_path']))
    ...
    while resume:
        ...
        for (x0,y0) in train_loader:
            ...
            sip1 = tdist.sample(x0.shape[0],iteration)[1]
            ...
            with torch.no_grad():
                ...
                unet_forward = lambda x,t : flatten_feats(clf(unet.module(x,t,rng_state=rng_state),return_feats=True))
                _, tangent = jvp(unet_forward, primals, tangents)
            ...
            F_xsip1 = flatten_feats(clf(unet(xsip1,sip1,rng_state=rng_state),return_feats=True))
            
            # Optimize UNet
            opt.zero_grad()
            w_ccd = 1/sip1.reshape(-1,1,1,1)
            loss_cm = (w_ccd * F_xsip1 * tangent).sum(dim=1).mean()
            loss = loss_cm
            loss.backward()
            opt.step()
