...
def main(cfg: DictConfig):
    ...
    ae = AutoEncoder().cuda()
    ae.load_state_dict(torch.load(cfg['ae_ckpt_path']),strict=False)
    ...
    while True:
        for (x0,y0) in train_loader:
            ...
            loss_dst = ae.distance(D_xsip1_cp,D_xsi,reduce_mean=True)
            loss_dst.backward()
            loss_dst_grad = normalize(D_xsip1_cp.grad.detach().clone())
            loss_dst_prox = (w_icm * 0.5 * (D_xsip1 - (D_xsip1-loss_dst_grad).detach()).square().sum(dim=(1,2,3))).mean()

            ru_factor = min(1.0,(iteration+1)/n_rampup_iter)
            loss = loss_dst_prox if (cfg['exp']['lmda1'] > 1000) else loss_cm + cfg['exp']['lmda1'] * ru_factor * loss_dst_prox
            loss.backward()
            opt.step()
            opt.zero_grad()
